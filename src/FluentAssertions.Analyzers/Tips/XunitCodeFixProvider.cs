    ...
    protected override CreateChangedDocument TryComputeFix(IInvocationOperation invocation, CodeFixContext context, TestingFrameworkCodeFixContext t, Diagnostic diagnostic)
    {
        switch (invocation.TargetMethod.Name)
        {
            ...
            case "Equal" when ArgumentsAreTypeOf(invocation, t.DateTime, t.DateTime, t.TimeSpan): // Assert.Equal(DateTime expected, DateTime actual, TimeSpan precision)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "BeCloseTo", argumentIndex: 1, argumentsToRemove: []);
            case "Equal" when ArgumentsAreGenericTypeOf(invocation, t.Identity, t.Identity, t.IEqualityComparerOfT1): // Assert.Equal<T>(T expected, T actual, IEqualityComparer<T> comparer)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertionWithOptionsLambda(invocation, context, "BeEquivalentTo", argumentIndex: 1, optionsIndex: 2);
            ...
            case "NotEqual" when ArgumentsAreTypeOf(invocation, t.DateTime, t.DateTime, t.TimeSpan): // Assert.NotEqual(DateTime expected, DateTime actual, TimeSpan precision)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "NotBeCloseTo", argumentIndex: 1, argumentsToRemove: []);
            case "NotEqual" when ArgumentsAreGenericTypeOf(invocation, t.Identity, t.Identity, t.IEqualityComparerOfT1): // Assert.NotEqual<T>(T expected, T actual, IEqualityComparer<T> comparer)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertionWithOptionsLambda(invocation, context, "NotBeEquivalentTo", argumentIndex: 1, optionsIndex: 2);
            ...
            case "Contains" when ArgumentsCount(invocation, arguments: 2): // Assert.Contains<T>(T expected, IEnumerable<T> collection)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "Contain", argumentIndex: 1, argumentsToRemove: []);
            ...
            case "DoesNotContain" when ArgumentsCount(invocation, arguments: 2): // Assert.DoesNotContain(string expectedSubstring, string? actualString)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "NotContain", argumentIndex: 1, argumentsToRemove: []);
            ...
            case "Matches" when ArgumentsAreTypeOf(invocation, t.Regex, t.String): // Assert.Matches(Regex regexPattern, string? actualString)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "MatchRegex", argumentIndex: 1, argumentsToRemove: []);
            ...
            case "DoesNotMatch" when ArgumentsAreTypeOf(invocation, t.Regex, t.String): // Assert.DoesNotMatch(Regex regexPattern, string? actualString)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "NotMatchRegex", argumentIndex: 1, argumentsToRemove: []);
            ...
            case "EndsWith" when ArgumentsAreTypeOf(invocation, t.String, t.String): // Assert.EndsWith(string expectedEndString, string? actualString)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "EndWith", argumentIndex: 1, argumentsToRemove: []);
            case "StartsWith" when ArgumentsAreTypeOf(invocation, t.String, t.String): // Assert.StartsWith(string expectedStartString, string? actualString)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "StartWith", argumentIndex: 1, argumentsToRemove: []);
            case "Subset" when ArgumentsCount(invocation, arguments: 2): // Assert.Subset<T>(ISet<T> expectedSubset, ISet<T> actualSet)
                return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "BeSubsetOf", argumentIndex: 1, argumentsToRemove: []);
            case "IsAssignableFrom" when ArgumentsCount(invocation, arguments: 1): // Assert.IsAssignableFrom<T>(object? @object)
                return DocumentEditorUtils.RenameGenericMethodToSubjectShouldGenericAssertion(invocation, context, "BeAssignableTo", argumentIndex: 0, argumentsToRemove: []);
            case "IsAssignableFrom" when ArgumentsCount(invocation, arguments: 2): // Assert.IsAssignableFrom(Type expectedType, object? @object) 
                {
                    ...
                    {
                        return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "BeAssignableTo", argumentIndex: 1, argumentsToRemove: []);
                    }

                    return DocumentEditorUtils.RenameMethodToSubjectShouldGenericAssertion(invocation, ImmutableArray.Create(typeOf.TypeOperand), context, "BeAssignableTo", argumentIndex: 1, argumentsToRemove: [0]);
                }
            case "IsNotAssignableFrom" when ArgumentsCount(invocation, arguments: 1): // Assert.IsNotAssignableFrom<T>(object? @object)
                return DocumentEditorUtils.RenameGenericMethodToSubjectShouldGenericAssertion(invocation, context, "NotBeAssignableTo", argumentIndex: 0, argumentsToRemove: []);
            case "IsNotAssignableFrom" when ArgumentsCount(invocation, arguments: 2): // Assert.IsNotAssignableFrom(Type expectedType, object? @object) 
                {
                    if (invocation.Arguments[0].Value is not ITypeOfOperation typeOf)
                    {
                        return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "NotBeAssignableTo", argumentIndex: 1, argumentsToRemove: []);
                    }

                    return DocumentEditorUtils.RenameMethodToSubjectShouldGenericAssertion(invocation, ImmutableArray.Create(typeOf.TypeOperand), context, "NotBeAssignableTo", argumentIndex: 1, argumentsToRemove: [0]);
                }
            ...
            case "IsType" when ArgumentsCount(invocation, arguments: 2): // Assert.IsType(Type expectedType, object? @object) 
                {
                    if (invocation.Arguments[0].Value is not ITypeOfOperation typeOf)
                    {
                        return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "BeOfType", argumentIndex: 1, argumentsToRemove: []);
                    }

                    return DocumentEditorUtils.RenameMethodToSubjectShouldGenericAssertion(invocation, ImmutableArray.Create(typeOf.TypeOperand), context, "BeOfType", argumentIndex: 1, argumentsToRemove: [0]);
                }
            ...
            case "IsNotType" when ArgumentsCount(invocation, arguments: 2): // Assert.IsNotType(Type expectedType, object? @object) 
                {
                    if (invocation.Arguments[0].Value is not ITypeOfOperation typeOf)
                    {
                        return DocumentEditorUtils.RenameMethodToSubjectShouldAssertion(invocation, context, "NotBeOfType", argumentIndex: 1, argumentsToRemove: []);
                    }

                    return DocumentEditorUtils.RenameMethodToSubjectShouldGenericAssertion(invocation, ImmutableArray.Create(typeOf.TypeOperand), context, "NotBeOfType", argumentIndex: 1, argumentsToRemove: [0]);
                }
        }
        return null;
    }
